# 🌙 TradeHive Market Data API Documentation
...
The TradeHive Market Data API (`src/agents/api.py`) provides access to real-time market data. The API is designed to be easy to use while providing powerful features for market analysis and automated trading.
...
```bash
# Create a .env file and add your TradeHive API key
TRADEHIVE_API_KEY=your_api_key_here
TRADEHIVE_API_BASE_URL=http://localhost:8000
```
...
api = TradeHiveAPI()  # Automatically loads API key from .env
```
...
---
*Built with 🌙 by TradeHive - Making trading data accessible and beautiful*
