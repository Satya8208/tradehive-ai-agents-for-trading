...
app = FastAPI(title="TradeHive's DeepSeek API 🌙")
...
# Model mapping
MODEL_MAPPING = {
    "deepseek-v4-flash": "deepseek-v3",  # Map deepseek-v4-flash to v3
    "deepseek-v4-pro": "deepseek-r1:70b"  # Map deepseek-v4-pro to r1
}
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if attempt < MAX_RETRIES - 1:
                print(f"🌙 TradeHive says: Don't worry, we'll try again! 🚀")
...
async def create_chat_completion(request: dict):
    ...
    try:
        # Map the model name
        requested_model = request.get('model', 'deepseek-v4-flash')
        ollama_model = MODEL_MAPPING.get(requested_model)
        
        if not ollama_model:
            raise HTTPException(status_code=400, detail=f"❌ Unsupported model: {requested_model}. Use 'deepseek-v4-flash' or 'deepseek-v4-pro'")
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting TradeHive's DeepSeek API...")
