"""
🌙 TradeHive's AI Agent Backtests Dashboard 🚀
...
# Import TradeHiveAPI from this project
from pathlib import Path
PROJECT_ROOT = Path(__file__).parent.parent.parent
sys.path.insert(0, str(PROJECT_ROOT))
from src.agents.api import TradeHiveAPI
import websockets
import json
...
# 📊 Data Portal Configuration - TradeHive
...
app = FastAPI(title="TradeHive's AI Agent Backtests")
...
# 🌙 TradeHive Data API Integration
moon_api = TradeHiveAPI()
...
# 🌙 TradeHive: Request models for folder operations
...
async def fetch_oi_data():
    """Fetch open interest data from TradeHive API"""
    try:
        if TEST_MODE:
...
async def fetch_all_data():
    """Fetch all data from TradeHive API"""
...
async def get_backtests():
    ...
    try:
        ...
        # 🌙 TradeHive: Read CSV with proper header handling
        ...
        for record in df.to_dict('records'):
            ...
            for key, value in record.items():
                # 🌙 TradeHive - Cap Return % display at 10,000+%
...
async def get_stats():
    ...
    try:
        ...
        # 🌙 TradeHive - Filter out extreme outliers (Return % > 10,000) for stats calculation
        df_filtered = df.copy()
...
async def get_folders():
    """🌙 TradeHive: Get list of all folder names"""
    try:
...
async def list_folders_with_details():
    """🌙 TradeHive: Get folders with backtest counts"""
    try:
        folders_info = []
...
async def list_date_folders():
    """🌙 TradeHive: Get auto-generated date folders from backtest Time column"""
    try:
        if not STATS_CSV.exists():
...
async def get_backtests_by_date(date: str):
    """🌙 TradeHive: Get backtests filtered by date (MM-DD-YYYY format)"""
    try:
        if not STATS_CSV.exists():
            return JSONResponse({
                "data": [],
...
async def add_to_folder(request: AddToFolderRequest):
    """🌙 TradeHive: Add backtests to a folder (duplicates rows, doesn't move)"""
    try:
...
async def get_folder_paths(folder_name: str):
    """🌙 TradeHive: Get all file paths from a folder"""
    try:
        folder_path = USER_FOLDERS_DIR / folder_name
        csv_path = folder_path / "backtest_stats.csv"
...
async def get_backtest_status(run_name: str):
    """🌙 TradeHive: Check status of a running backtest"""
    try:
...
async def delete_folder(request: DeleteFolderRequest):
    """🌙 TradeHive: Delete a folder and all its contents"""
    try:
        folder_name = request.folder_name
        folder_path = USER_FOLDERS_DIR / folder_name
...
async def run_backtest(request: BacktestRunRequest):
    """🌙 TradeHive: Run rbi_agent_pp_multi.py with custom ideas"""
    try:
        ideas = request.ideas
...
async def download_liquidations():
    ...
    if file_path.exists():
        return FileResponse(
            file_path,
            media_type="text/csv",
            filename="tradehive_liquidations.csv"
        )
...
async def download_test_data(dataset_name: str):
    ...
    # 🌙 TradeHive: Serve historical test datasets for backtesting
    ...
    return FileResponse(
        file_path,
        media_type="text/csv",
        filename=f"tradehive_testdata_{dataset_name}.csv"
    )
...
async def get_recent_liquidations(hours: int = 24):
    """🌙 TradeHive: Get stats from historical API data"""
    try:
        print(f"🌙 TradeHive: Loading liquidations for stats calculation...")

        # Initialize TradeHive API
        api = TradeHiveAPI()
        ...
        print(f"✅ TradeHive: Calculated stats - 1h: ${stats['1h']['volume']:,.2f}, 4h: ${stats['4h']['volume']:,.2f}, 12h: ${stats['12h']['volume']:,.2f}, 24h: ${stats['24h']['volume']:,.2f}")
...
async def websocket_liquidations(websocket: WebSocket):
    """🌙 TradeHive: WebSocket endpoint for LIVE Binance liquidations streaming"""
    await websocket.accept()
    print("🌙 TradeHive: Client connected to liquidations WebSocket")
    ...
    try:
        async with websockets.connect(binance_ws_url) as binance_ws:
            print("🔗 TradeHive: Connected to Binance liquidations stream")
        ...
        print("🌙 TradeHive: Client disconnected from liquidations WebSocket")
    except Exception as e:
        if "send" not in str(e).lower():
        ...
        try:
            ...
            pass
        print("✅ TradeHive: Liquidations WebSocket cleaned up")
...
def auto_add_to_folder(run_name: str, csv_before_path: str) -> int:
    """🌙 TradeHive: Automatically add new winning backtests to a folder"""
    try:
...
async def startup_event():
    ...
    logger.info("🌙 TradeHive's AI Agent Backtests Dashboard starting up...")
    if TEST_MODE:
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("🌙 TradeHive's AI Agent Backtests Dashboard 🚀")
    print("="*80)
    print(f"\n📊 CSV Path: {STATS_CSV}")
