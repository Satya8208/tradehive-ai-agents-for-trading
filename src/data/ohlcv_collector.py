"""
🌙 TradeHive's OHLCV Data Collector
...
def collect_token_data(token, days_back=DAYSBACK_4_DATA, timeframe=DATA_TIMEFRAME, exchange="SOLANA"):
    ...
    """
    cprint(f"\n🤖 TradeHive's AI Agent fetching data for {token}...", "white", "on_blue")
    ...
    try:
        ...
        if data is None or data.empty:
            cprint(f"❌ TradeHive's AI Agent couldn't fetch data for {token}", "white", "on_red")
            return None

        cprint(f"📊 TradeHive's AI Agent processed {len(data)} candles for analysis", "white", "on_blue")
        ...
        cprint(f"💾 TradeHive's AI Agent cached data for {token[:4]}", "white", "on_green")
        
        return data
        
    except Exception as e:
        cprint(f"❌ TradeHive's AI Agent encountered an error: {str(e)}", "white", "on_red")
        return None
...
def collect_all_tokens(tokens=None, days_back=None, timeframe=None, exchange="SOLANA"):
    ...
    cprint("\n🔍 TradeHive's AI Agent starting market data collection...", "white", "on_blue")
    ...
    cprint("\n✨ TradeHive's AI Agent completed market data collection!", "white", "on_green")
...
if __name__ == "__main__":
    try:
        ...
        print("\n👋 TradeHive OHLCV Collector shutting down gracefully...")
    except Exception as e:
        print(f"❌ Error: {str(e)}")
