...
# Model settings
MODEL_NAME = "gpt-5.4-mini"  # GPT-5.4 Mini - Fast and capable
...
* how can i get in touch with you (tradehive)?the best way is to catch me on a live stream. if its about business you can send me a short email at the project Discord i can't get back to everyone, so please pitch short and concisely
* can you build a bot for me?probably not, i code live every day on youtube so i can show my code to as many people as possible to help them. if you have a project that you'd like to hire me to do & are ok with some of it being shown on youtube, feel free to email me here: the project Discordi would much rather teach you how to fish, opposed to just giving you a meal. that's why i teach how to automate your trading in the bootcamp
...
* can i pay for the bootcamp in crypto?yes, if you are looking to sign up for the lifetime package. unfortunately, there is no way to collect subscriptions in crypto so we do lifetime only. you can email the project Discord for the address to send crypto to set up your bootcamp account.
...
* whats your github? @tradehiveonyt
"""
...
def needs_knowledge_base(question):
    """Check if the question is about TradeHive, Algo Trade Camp, or specific offerings"""
    # Convert to lowercase for matching
    question = question.lower()

    # Keywords that indicate we need to verify against knowledge base
    tradehive_keywords = {
        'tradehive', 'tradehive', 'moon', 'bootcamp', 'algo trade camp', 'algotradecamp',
    ...
    return any(keyword in question for keyword in tradehive_keywords)
...
async def test_conversation():
    ...
    try:
        ...
        # Welcome message
        cprint("\n" + "═" * 50, "green")
        cprint("🌟 Starting TradeHive's AI Voice Assistant! 🌙", "green")
        cprint("Press Ctrl+C to end the conversation", "yellow")
        cprint("═" * 50, "green")

        # Initial greeting
        greeting = "Hello! This is TradeHive's AI Assistant. How can I help you today? 🌙"
        cprint("\n🤖 AI: " + greeting, "green")
        ...
        conversation_history = [
            {"role": "system", "content": f"""You are TradeHive's friendly AI assistant. Keep responses very short and concise (1-2 sentences max). Add emojis to make responses fun and engaging.

Use this knowledge base for TradeHive specific questions: {KNOWLEDGE_BASE}

Key guidelines:
- Use knowledge base for TradeHive/Algo Trade Camp specific questions
...
"""},
            ...
            if not recorder.audio_queue.empty():
                ...
                if transcript.strip():
                    ...
                    if not can_answer:
                        ...
                        unknown_response = "I apologize, but I'm not sure about that. Please email the project Discord and we'll get that answered ASAP! 🌙✉️"
        ...
        cprint("\n\n👋 Call ended. TradeHive's AI signing off! 🌙", "yellow")
...
if not TESTING_MODE:
    ...
    def answer_call():
        ...
        # Welcome message
        resp.say("Welcome to TradeHive's A.I. Assistant! 🌙", voice=VOICE_NAME)
    ...
    def handle_input():
        ...
        try:
            ...
            response = openai.chat.completions.create(
                model=MODEL_NAME,
                messages=[
                    {"role": "system", "content": "You are TradeHive's friendly AI assistant. Keep responses concise and clear for phone calls."},
...
async def process_message(message):
    ...
    try:
        ...
        if not can_answer:
            ...
            return "I apologize, but I'm not sure about that. Please email the project Discord and we'll get that answered ASAP! 🌙✉️"
...
def start_server():
    ...
    try:
        cprint("\n🚀 Starting TradeHive's Phone Agent...", "green")
