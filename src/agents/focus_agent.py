...
# available free while tradehive is streaming: https://www.youtube.com/@tradehiveonyt
...
# OpenAI Models:
# - "gpt-5.5": Latest OpenAI flagship in official docs
# - "gpt-5.4-mini": Smaller, faster current GPT-5 family model
...
# DeepSeek Models:
# - "deepseek-v4-flash": Fast chat model
# - "deepseek-v4-pro": Enhanced reasoning model
...
TEST_TRANSCRIPT = """Hey TradeHive here, I'm working on implementing the new trading algorithm using Python.
...
FOCUS_PROMPT = """You are TradeHive's Focus AI Agent. Your task is to analyze the following transcript and rate focus.
...
EXAMPLE RESPONSE:
8/10
Keep crushing that code, TradeHive! Your focus is leading to amazing results.
...
class FocusAgent:
    def __init__(self):
        ...
        if MODEL_TYPE == "openai":
            ...
            cprint(f"\n💫 TradeHive's Focus Agent using OpenAI!", "green")
        ...
        cprint("🎯 TradeHive's Focus Agent initialized!", "green")

        self.is_recording = False
        self.current_transcript = []

        # Add data directory path
        self.data_dir = Path("/Users/md/Dropbox/dev/github/tradehive-ai-agents-for-trading/src/data")
    ...
    def _check_schedule(self):
        ...
        if not (SCHEDULE_START <= current_time <= SCHEDULE_END):
            cprint(f"\n🌙 TradeHive's Focus Agent is scheduled to run between {SCHEDULE_START.strftime('%I:%M %p')} and {SCHEDULE_END.strftime('%I:%M %p')}", "yellow")
        ...
        try:
            ...
            if MODEL_TYPE == "ollama":
                ...
                response = self.model.generate_response(
                    system_prompt="You are TradeHive's Focus AI. You analyze focus and provide ratings. NO MARKDOWN OR FORMATTING. RESPOND WITH EXACTLY TWO LINES: A SCORE LINE (X/10) AND ONE SINGLE ENCOURAGING SENTENCE.",
                ...
                return 5, "Keep crushing it TradeHive! Your focus is amazing!"

        except Exception as e:
            cprint(f"❌ Error analyzing focus: {str(e)}", "red")
            return 5, "Error analyzing focus, but keep going TradeHive!"  # Always return a tuple
    ...
    def _create_focus_log(self):
        ...
        cprint("🌟 TradeHive's Focus History log created!", "green")
    ...
    def run(self):
        ...
        cprint("\n🎯 TradeHive's Focus Agent starting with voice monitoring...", "cyan")
