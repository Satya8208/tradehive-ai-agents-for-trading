...
# Master prompts
TITLE_MASTER_PROMPT = """You are Gilfoyle from Silicon Valley generating stream titles. Based on TradeHive's livestream transcript, generate exactly {num_titles} self-deprecating, sarcastic titles.

Rules:
- Each title MUST be 10 words or less
- Make fun of TradeHive (the streamer) in a funny way
...
- Act like TradeHive is trying too hard or missing obvious things
...
THUMBNAIL_MASTER_PROMPT = """Create a YouTube thumbnail for TradeHive's coding livestream. The stream is about: {context}
...
The thumbnail should visually represent TradeHive building trading algorithms live."""
...
class StreamAgent:
    def __init__(self):
        ...
        self.model = model_factory.get_model("openai", "gpt-5.4-mini")
    ...
    def generate_thumbnails(self, transcript, titles):
        ...
        try:
            ...
            context = f"TradeHive is coding trading algorithms. Topics discussed: {transcript[:500]}... Key themes from titles: {', '.join(titles[:3])}"
