"""
🌙 TradeHive's Polymarket Prediction Market Agent
...
# 🌙 TradeHive - Market category filters (case-insensitive)
...
# 🌙 TradeHive - AI Prompts (customize these for your own edge!)
# ==============================================================================
...
CONSENSUS_PICKS_CSV = os.path.join(DATA_FOLDER, "consensus_picks.csv")  # 🌙 TradeHive - Top consensus picks only
...
class PolymarketAgent:
    ...
    def _load_predictions(self):
        ...
        # Create new DataFrame with required columns
        # 🌙 TradeHive - Link column at END for clickable CSVs in Excel/Numbers
        return pd.DataFrame(columns=[
    ...
    def should_ignore_market(self, title):
        """🌙 TradeHive - Check if market should be ignored based on category keywords
    ...
    def on_ws_message(self, ws, message):
        """🌙 TradeHive - Handle incoming WebSocket messages"""
        try:
            ...
            if isinstance(data, dict):
                ...
                if data.get('type') == 'subscribed':
                    cprint("✅ TradeHive WebSocket subscribed successfully to live trades!", "green")
                ...
                if topic == 'activity' and msg_type == 'orders_matched':
                    ...
                    # 🌙 TradeHive - Check if we should ignore this market category
                    ...
                    if usd_amount >= MIN_TRADE_SIZE_USD and not self.is_near_resolution(price):
                        ...
                        # 🌙 TRADEHIVE - Process this trade immediately
                        trade_data = {
            ...
            cprint(f"⚠️ TradeHive - Error processing WebSocket message: {e}", "yellow")

    def on_ws_error(self, ws, error):
        """🌙 TradeHive - Handle WebSocket errors"""
        cprint(f"❌ TradeHive WebSocket Error: {error}", "red")

    def on_ws_close(self, ws, close_status_code, close_msg):
        """🌙 TradeHive - Handle WebSocket close"""
        self.ws_connected = False
        cprint(f"\n🔌 TradeHive WebSocket connection closed: {close_status_code} - {close_msg}", "yellow")
        cprint("Reconnecting in 5 seconds...", "cyan")
        time.sleep(5)
        self.connect_websocket()

    def on_ws_open(self, ws):
        """🌙 TradeHive - Handle WebSocket open - send subscription"""
        cprint("🔌 TradeHive WebSocket connected!", "green")
        ...
        cprint(f"📡 TradeHive sending subscription for live trades...", "cyan")
        ws.send(json.dumps(subscription_msg))

        # Set connected flag immediately after sending subscription
        self.ws_connected = True
        cprint("✅ TradeHive subscription sent! Waiting for trades...", "green")
    ...
    def connect_websocket(self):
        """🌙 TradeHive - Connect to Polymarket WebSocket"""
        cprint(f"🚀 TradeHive connecting to {WEBSOCKET_URL}...", "cyan")
        ...
        cprint("✅ TradeHive WebSocket thread started!", "green")

    def fetch_historical_trades(self, hours_back=None):
        """🌙 TradeHive - Fetch historical trades from Polymarket API on startup
        ...
        try:
            cprint(f"\n📡 TradeHive fetching historical trades (last {hours_back}h)...", "yellow")
        ...
        for market_id, trade in unique_markets.items():
            try:
                ...
                if market_id in self.markets_df['market_id'].values:
                    # 🌙 TradeHive - UPDATE existing market with new trade data (fresh odds!)
    ...
    def get_ai_predictions(self):
        ...
        # 🌙 TradeHive - Conditionally include price info based on config
...
Provide predictions for each market in the specified format."""
        ...
        if USE_SWARM_MODE and self.swarm:
            ...
            cprint("📡 TradeHive sending prompts to swarm...", "cyan")
            ...
            # 🌙 TradeHive - Run final consensus AI to pick top 3 markets
            ...
            # 🌙 TradeHive - Mark analyzed markets with timestamp
    ...
    def _mark_markets_analyzed(self, markets, analysis_timestamp):
        """🌙 TradeHive - Mark markets as analyzed with timestamp
    ...
    def _save_swarm_predictions(self, analysis_run_id, analysis_timestamp, markets, swarm_result):
        """🌙 TradeHive - Save swarm predictions to CSV database (one row per market)
                ...
                for line in lines:
                    ...
                    if 'MARKET' in line_upper and ':' in line:
                        try:
                            ...
                            # 🌙 TradeHive - Validate market number is within range
            ...
            for market_num, predictions in market_predictions.items():
                ...
                if 1 <= market_num <= len(markets_list):
                    ...
                    record = {
                        ...
                        'opus_prediction': predictions.get('opus', 'N/A'),  # 🌙 TradeHive - Opus 4.7 predictions
                        ...
                        'market_link': market_link  # 🌙 TradeHive - Link at end for clickable CSVs
                    }
    ...
    def _calculate_polymarket_consensus(self, swarm_result, markets_df):
        """
        🌙 TradeHive - Calculate consensus from individual swarm responses for Polymarket predictions
    ...
    def _get_top_consensus_picks(self, swarm_result, markets_df):
        """
        🌙 TradeHive - Use consensus AI to identify top 3 markets with strongest agreement
        ...
        try:
            ...
            # Use Claude 4.6 Sonnet for consensus (fast and reliable)
            consensus_model = ModelFactory().get_model('claude', 'claude-sonnet-4-6')
            ...
            cprint(f"🏆 TOP {TOP_MARKETS_COUNT} CONSENSUS PICKS - TRADEHIVE AI RECOMMENDATION", "white", "on_blue", attrs=['bold'])
            ...
            # 🌙 TradeHive - Save consensus picks to dedicated CSV
    ...
    def _save_consensus_picks_to_csv(self, consensus_response, markets_df):
        """
        🌙 TradeHive - Save top consensus picks to dedicated CSV (append-only)
        ...
        try:
            ...
            # Convert to records for CSV (🌙 TradeHive - link at END for clickable CSVs)
            records = []
            for pick in picks:
                record = {
                    'timestamp': timestamp,
            ...
            # Load or create consensus picks CSV (🌙 TradeHive - link at END for clickable CSVs)
    ...
    def status_display_loop(self):
        """🌙 TradeHive - Display status updates every 30 seconds"""
        ...
        while True:
            try:
                ...
                # 🌙 TradeHive - Count markets with FRESH TRADES that are also ELIGIBLE
                now = datetime.now()
                ...
                cprint(f"\n{'='*60}", "cyan")
                cprint(f"📊 TradeHive Status @ {datetime.now().strftime('%H:%M:%S')}", "cyan", attrs=['bold'])
                cprint(f"{'='*60}", "cyan")
    ...
    def analysis_cycle(self):
        ...
        # 🌙 TradeHive - Skip if no markets exist yet
        if total_markets == 0:
        ...
        # 🌙 TradeHive - Count markets with FRESH TRADES that are also ELIGIBLE for re-analysis
        now = datetime.now()
        cutoff_time = now - timedelta(hours=REANALYSIS_HOURS)
        ...
        if should_analyze:
            ...
            # 🌙 TradeHive - Update analysis run timestamp
    ...
    def analysis_loop(self):
        """🌙 TradeHive - Continuously check for new markets to analyze (runs immediately!)"""
        ...
        # 🌙 TradeHive - Run first analysis IMMEDIATELY (no waiting!)
        cprint("🚀 TradeHive running first analysis immediately...\n", "yellow", attrs=['bold'])
...
def main():
    """🌙 TradeHive Main - WebSocket real-time data + AI analysis threads"""
    cprint("\n" + "="*80, "cyan")
    cprint("🌙 TradeHive's Polymarket Agent - WebSocket Edition!", "cyan", attrs=['bold'])
    cprint("="*80, "cyan")
    ...
    # 🌙 TradeHive - Fetch historical trades on startup to populate database
    cprint("\n" + "="*80, "yellow")
    cprint(f"📜 TradeHive fetching historical data from last {LOOKBACK_HOURS} hours...", "yellow", attrs=['bold'])
    cprint("="*80, "yellow")
    ...
    # Start threads
    try:
        cprint("🚀 TradeHive starting threads...\n", "green", attrs=['bold'])
        status_thread.start()
        analysis_thread.start()

        # Keep main thread alive
        cprint("✨ TradeHive WebSocket + AI running! Press Ctrl+C to stop.\n", "green", attrs=['bold'])
        while True:
            time.sleep(1)

    except KeyboardInterrupt:
        cprint("\n\n" + "="*80, "yellow")
        cprint("⚠️ TradeHive Polymarket Agent stopped by user", "yellow", attrs=['bold'])
        cprint("="*80 + "\n", "yellow")
        sys.exit(0)
