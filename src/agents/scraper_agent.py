...
# 🌙 TradeHive: Dynamic path calculation
...
# Import TradeHive's agents and models
...
class ScraperAgent:
    """🌙 TradeHive's Scraper Agent for website analysis"""
    ...
    def __init__(self, use_swarm: bool = USE_SWARM, prompt: Optional[str] = None):
        ...
        cprint("\n" + "="*60, "cyan")
        cprint("🌙 TradeHive's Scraper Agent Initialized 🌙", "cyan", attrs=['bold'])
        cprint("="*60, "cyan")
        ...
        while True:
            try:
                ...
                if user_input.lower() in ['quit', 'exit', 'q']:
                    cprint("\n👋 Goodbye TradeHive! 🌙", "green", attrs=['bold'])
                    break
                ...
                cprint("👋 Goodbye TradeHive! 🌙", "cyan", attrs=['bold'])
                break
            except Exception as e:
                cprint(f"\n❌ Error: {str(e)}", "red")
