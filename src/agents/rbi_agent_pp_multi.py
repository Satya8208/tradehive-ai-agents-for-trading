"""
🌙 TradeHive's RBI AI v3.0 PARALLEL PROCESSOR + MULTI-DATA TESTING 🚀
...
3. External dependency: tradehive-trading-bots repo (clone as sibling: ../tradehive-trading-bots)
...
# 🌙 TradeHive: Calculate project root dynamically (works on any system!)
...
import sys
import argparse  # 🌙 TradeHive: For command-line args
...
AI_TEMPERATURE = 0.7
AI_MAX_TOKENS = 8000  # 🌙 TradeHive: DeepSeek limit is 8192, using 8000 to be safe

# Import model factory with proper path handling
import sys
sys.path.append(str(PROJECT_ROOT))  # 🌙 TradeHive: Dynamic path that works on any system!
...
# ============================================
# 📁 STRATEGY SOURCE CONFIGURATION - TradeHive
# ============================================
...
STRATEGIES_FOLDER = DATA_DIR / "web_search_research" / "final_strategies"  # 🌙 TradeHive: Dynamic path!
...
api_lock = Lock()
file_lock = Lock()
date_lock = Lock()  # 🌙 TradeHive: Lock for date checking/updating

# Rate limiter
rate_limiter = Semaphore(MAX_PARALLEL_THREADS)

# 🌙 TradeHive's Model Configurations
...
# - OpenAI: openai/gpt-5.5, openai/gpt-5.5-pro, openai/gpt-5.2, openai/gpt-5.2-pro
...
# ============================================
# 🌙 TradeHive's MODEL SELECTION - Easy Switch!
# ============================================
...
# 🧠 RESEARCH - DeepSeek V4 Pro (shows thinking process, great for analysis)
# RESEARCH_CONFIG = {"type": "claude", "name": "claude-opus-4-7"}
...
# BACKTEST_CONFIG = {"type": "claude", "name": "claude-opus-4-7"}
# BACKTEST_CONFIG = {"type": "xai", "name": "grok-4-fast-reasoning"}
BACKTEST_CONFIG = {"type": "deepseek", "name": "deepseek-coder"}

# 🐛 DEBUGGING - DeepSeek V4 Pro (thinking process helps debug)
# DEBUG_CONFIG = {"type": "claude", "name": "claude-opus-4-7"}
# DEBUG_CONFIG = {"type": "xai", "name": "grok-4-fast-reasoning"}
DEBUG_CONFIG = {"type": "deepseek", "name": "deepseek-v4-pro"}

# 📦 PACKAGE CHECK - DeepSeek Coder (understands imports/dependencies)
# PACKAGE_CONFIG = {"type": "claude", "name": "claude-opus-4-7"}
...
# OPTIMIZE_CONFIG = {"type": "claude", "name": "claude-opus-4-7"}
...
SAVE_IF_OVER_RETURN = 1.0  # Save backtest to CSV and folders if return > this % (TradeHive's threshold!)
CONDA_ENV = "tflow"
...
# 🌙 TradeHive: Date tracking for always-on mode - will update when date changes!
...
# 🌙 TradeHive: These will be updated dynamically when date changes
TODAY_DIR = None
RESEARCH_DIR = None
BACKTEST_DIR = None
PACKAGE_DIR = None
...
STATS_CSV = DATA_DIR / "backtest_stats.csv"  # TradeHive's stats tracker!
...
def update_date_folders():
    """
    🌙 TradeHive's Date Folder Updater!
...
# 🌙 TradeHive: Initialize folders on startup!
update_date_folders()
...
# ============================================
# 📄 PDF & YOUTUBE EXTRACTION - TradeHive
# ============================================

def get_youtube_transcript(video_id, thread_id):
    """Get transcript from YouTube video - TradeHive"""
    try:
        try:
        ...
        # 🌙 TradeHive: Using youtube-transcript-api v1.2.3+ API
        ...
        # 🌙 TradeHive: Print first 300 characters for verification
...
def get_pdf_text(url, thread_id):
    """Extract text from PDF URL - TradeHive"""
    try:
        try:
            import PyPDF2
        except ImportError:
...
def extract_youtube_id(url):
    """Extract video ID from YouTube URL - TradeHive"""
    try:
        if "v=" in url:
...
def extract_content_from_url(idea: str, thread_id: int) -> str:
    """
    🌙 TradeHive: Extract content from PDF or YouTube URLs
...
RESEARCH_PROMPT = """
You are TradeHive's Research AI 🌙
...
BACKTEST_PROMPT = f"""
You are TradeHive's Backtest AI 🌙
...
4. Print entry/exit signals with TradeHive themed messages
...
Always add plenty of TradeHive themed debug prints with emojis to make debugging easier! 🌙 ✨ 🚀
...
```python
# 🌙 TRADEHIVE'S MULTI-DATA TESTING FRAMEWORK 🚀
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('src/data/market_data/binance/BTCUSDT/BTC-USDT-5m.csv')  # 🌙 TradeHive: Relative path from project root!
    ...
    # 🌙 CRITICAL: Print full stats for TradeHive's parser!
    print("\\n" + "="*80)
    print("📊 BACKTEST STATISTICS (TradeHive's Format)")
    print("="*80)
    print(stats)
    print("="*80 + "\\n")

    # THEN: Run multi-data testing
    # 🌙 TradeHive: EXTERNAL DEPENDENCY - Requires tradehive-trading-bots repo
    # Clone from: https://github.com/tradehiveonyt/tradehive-trading-bots
    # Expected location: ../tradehive-trading-bots (sibling to this repo)
    external_backtests = str(Path(__file__).parent.parent.parent.parent / 'tradehive-trading-bots' / 'backtests')
    sys.path.append(external_backtests)
    from multi_data_tester import test_on_all_data

    print("\\n" + "="*80)
    print("🚀 TRADEHIVE'S MULTI-DATA BACKTEST - Testing on 25+ Data Sources!")
    print("="*80)
...
Return the complete fixed code with TradeHive themed debug prints! 🌙 ✨
ONLY SEND BACK CODE, NO OTHER TEXT.
"""

PACKAGE_PROMPT = """
You are TradeHive's Package AI 🌙
...
Return the complete fixed code with proper TradeHive themed debug prints! 🌙 ✨
ONLY SEND BACK CODE, NO OTHER TEXT.
"""

OPTIMIZE_PROMPT = """
You are TradeHive's Optimization AI 🌙
...
- Keep all TradeHive debug prints
...
Return the COMPLETE optimized code with TradeHive themed comments explaining what you improved! 🌙 ✨
ONLY SEND BACK CODE, NO OTHER TEXT.
"""
...
def parse_all_stats_from_output(stdout: str, thread_id: int) -> dict:
    """
    🌙 TradeHive's Stats Parser - Extract all key stats from backtest output!
...
def log_stats_to_csv(strategy_name: str, thread_id: int, stats: dict, file_path: str, data_source: str = "ideas.txt") -> None:
    """
    🌙 TradeHive's CSV Logger - Thread-safe stats logging!
            ...
            with open(STATS_CSV, 'a', newline='') as f:
                ...
                if not file_exists:
                    writer.writerow([
                        ...
                        'Sharpe Ratio',
                        'Sortino Ratio',
                        'Exposure %',  # 🌙 TradeHive: Added Exposure Time
                        'EV %',  # 🌙 TradeHive: Changed from Expectancy %
                        'Trades',  # 🌙 TradeHive: Added # Trades
                        'File Path',
                        'Data',  # 🌙 TradeHive: Changed from Data Source
                        'Time'   # 🌙 TradeHive: Changed from Timestamp
                    ])
                    thread_print("📝 Created new stats CSV with headers", thread_id, "green")

                # Write stats row
                # 🌙 TradeHive: Format time as "10/25 06:30"
                ...
                writer.writerow([
                    ...
                    stats.get('exposure', 'N/A'),  # 🌙 TradeHive: Added Exposure %
                    stats.get('expectancy', 'N/A'),
                    stats.get('trades', 'N/A'),  # 🌙 TradeHive: Added # Trades
                    str(file_path),
                    data_source,
                    timestamp
                ])
...
def parse_and_log_multi_data_results(strategy_name: str, thread_id: int, backtest_file_path: Path) -> None:
    """
    🌙 TradeHive's Multi-Data Results Parser!
    ...
    try:
        ...
        for idx, row in passing_results.iterrows():
            stats = {
                ...
                'exposure': row.get('Exposure_Time_%', None),  # 🌙 TradeHive: Added Exposure % (matches multi_data_tester.py column name)
                'expectancy': row.get('Expectancy_%', None),
                'trades': row.get('Trades', None)  # 🌙 TradeHive: Added # Trades
            }
...
def save_backtest_if_threshold_met(code: str, stats: dict, strategy_name: str, iteration: int, thread_id: int, phase: str = "debug") -> bool:
    """
    🌙 TradeHive's Threshold Checker - Save backtests that pass the return threshold!
    ...
    try:
        ...
        # Save to FINAL folder (same logic per TradeHive's request)
    ...
    with file_lock:
        if not PROCESSED_IDEAS_LOG.exists():
            ...
            with open(PROCESSED_IDEAS_LOG, 'w', encoding='utf-8') as f:
                f.write("# TradeHive's RBI AI - Processed Ideas Log 🌙\n")
...
def clean_model_output(output, content_type="text"):
    ...
    # 🌙 TradeHive: Final cleanup - strip any remaining markdown fences
...
def debug_backtest(backtest_code, error_message, strategy_name, thread_id, iteration=1):
    ...
    if output:
        ...
        # 🌙 TradeHive: Save debug iterations to BACKTEST_DIR, not FINAL
...
def process_trading_idea_parallel(idea: str, thread_id: int) -> dict:
    ...
    """
    try:
        # 🌙 TradeHive: Check if date has changed and update folders!
        update_date_folders()

        thread_print(f"🚀 Starting processing", thread_id, attrs=['bold'])

        # 🌙 TradeHive: Extract content from PDF/YouTube if URL provided
            ...
            if execution_result['success']:
                if has_nan_results(execution_result):
                    ...
                    if debug_iteration < MAX_DEBUG_ITERATIONS:
                        ...
                        # 🌙 TradeHive: Update to match new debug file location
                    ...
                    # 🌙 TradeHive: Parse ALL stats, not just return!
                    ...
                    # 🌙 TradeHive: Check threshold and save if met!
                    ...
                    # 🌙 TradeHive: Parse and log multi-data results!
                    ...
                    if current_return >= TARGET_RETURN:
                        ...
                        # 🌙 TradeHive: Save to OPTIMIZATION_DIR for target hits
                        ...
                        while optimization_iteration < MAX_OPTIMIZATION_ITERATIONS:
                            ...
                            # 🌙 TradeHive: Parse ALL stats from optimization!
                            ...
                            if new_return > best_return:
                                ...
                                # 🌙 TradeHive: Parse and log multi-data results from optimization!
                                thread_print("🔍 Checking for multi-data test results...", thread_id, "cyan")
                                parse_and_log_multi_data_results(
                                    strategy_name,
                                    thread_id,
...
def get_strategies_from_files():
    """🌙 TradeHive: Read all .md and .txt files from STRATEGIES_FOLDER"""
    strategies = []
...
def idea_monitor_thread(idea_queue: Queue, queued_ideas: set, queued_lock: Lock, stop_flag: dict):
    """🌙 TradeHive: Producer thread - monitors ideas.txt OR strategy files and queues new ideas"""
    global IDEAS_FILE

    while not stop_flag.get('stop', False):
        try:
            # 🌙 TradeHive: Check which mode we're in
...
def worker_thread(worker_id: int, idea_queue: Queue, queued_ideas: set, queued_lock: Lock, stats: dict, stop_flag: dict):
    """🌙 TradeHive: Consumer thread - processes ideas from queue"""
    while not stop_flag.get('stop', False):
        try:
...
def main(ideas_file_path=None, run_name=None):
    ...
    # 🌙 TradeHive: Use custom ideas file if provided
    global IDEAS_FILE
    if ideas_file_path:
        IDEAS_FILE = Path(ideas_file_path)

    cprint(f"\n{'='*60}", "cyan", attrs=['bold'])
    cprint(f"🌟 TradeHive's RBI AI v3.0 PARALLEL PROCESSOR + MULTI-DATA 🚀", "cyan", attrs=['bold'])
    ...
    # 🌙 TradeHive: Show VERY CLEAR configuration mode
    cprint(f"\n{'='*60}", "white", attrs=['bold'])
    if STRATEGIES_FROM_FILES:
    ...
    # 🌙 TradeHive: CONTINUOUS QUEUE MODE
    ...
    try:
        while True:
            ...
            # 🌙 TradeHive: Check for date changes periodically (even when idle!)
            update_date_folders()
...
if __name__ == "__main__":
    # 🌙 TradeHive: Parse command-line arguments for custom ideas file and run name
    parser = argparse.ArgumentParser(description="TradeHive's RBI Agent - Parallel Backtest Processor")
