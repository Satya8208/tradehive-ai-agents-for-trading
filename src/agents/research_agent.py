...
You are TradeHive's Trading Strategy Idea Generator 🌙
...
MODELS = [
    ...
    {"type": "deepseek", "name": "deepseek-v4-pro"}
]
...
def animate_tradehive():
    """Show a fun TradeHive animation - terminal friendly with background colors"""
    tradehive = [
        "  __  __                         ____                 ",
...
def setup_files():
    ...
    if not IDEAS_TXT.exists():
        ...
        with open(IDEAS_TXT, 'w') as f:
            f.write("# TradeHive's Trading Strategy Ideas 🌙\n")
...
def generate_idea(model_config):
    ...
    try:
        # Fun animated header
        print("\n" + "=" * min(60, TERM_WIDTH))
        cprint(f" 🧙‍♂️ TRADEHIVE'S IDEA GENERATOR 🧙‍♂️ ", "white", "on_magenta")
...
def log_idea(idea, model_config):
    ...
    for phase in MOON_PHASES:
        clear_line()
        print(f"\r{colored(' ' + phase + ' Saving to TradeHive database... ', 'white', 'on_magenta')}", end="", flush=True)
        time.sleep(0.3)  # Slower animation
    print()
...
def run_idea_generation_loop(interval=10):
    ...
    animate_tradehive()
    time.sleep(0.5)  # Pause for readability
    cprint("\n🌟 TRADEHIVE'S RESEARCH AGENT ACTIVATED! 🌟", "white", "on_magenta")
    time.sleep(0.5)  # Pause for readability
    ...
    try:
        ...
        cprint("\n👋 TRADEHIVE'S RESEARCH AGENT SHUTTING DOWN...", "white", "on_yellow")
        ...
        cprint("\n🌙 Thank you for using TradeHive's Research Agent! 🌙", "white", "on_magenta")
    except Exception as e:
...
def test_run(num_ideas=1, interval=10):
    ...
    animate_tradehive()
    time.sleep(0.5)  # Pause for readability
    cprint("\n🧪 TRADEHIVE'S RESEARCH AGENT - TEST MODE", "white", "on_magenta")
    time.sleep(0.5)  # Pause for readability
