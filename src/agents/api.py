...
2. Create a .env file in your project root:
   ```
   TRADEHIVE_API_KEY=your_api_key_here
   ```

3. Basic Usage:
   ```python
   from agents.api import TradeHiveAPI

   # Initialize with env variable (recommended)
   api = TradeHiveAPI()

   # Or initialize with direct key
   api = TradeHiveAPI(api_key="your_key_here")
...
- get_copybot_follow_list(): Get TradeHive's personal copy trading follow list (for reference only - DYOR!)
...
- CopyBot Data: TradeHive's personal trading signals (use as reference only, always DYOR!)
...
5. The copybot follow list is TradeHive's personal list and should not be used alone

Need an API key? for a limited time, bootcamp members get free api keys for claude, openai, helius, birdeye & quant elite gets access to the tradehive api. join here: https://algotradecamp.com
"""
...
class TradeHiveAPI:
    def __init__(self, api_key=None, base_url=None):
        ...
        self.api_key = api_key or os.getenv('TRADEHIVE_API_KEY')
        self.base_url = base_url or os.getenv("TRADEHIVE_API_BASE_URL", "http://localhost:8000")
    ...
    def _fetch_liquidation_chunked(self, limit=None):
        """
        🌙 TradeHive Enhanced Liquidation Data Download
        ...
        try:
            if limit is not None:
                ...
                print(f"🎯 TradeHive: Getting last {limit:,} liquidation records...")
                ...
                if df is not None:
                    ...
                    if timestamp_col:
                        ...
                        print(f"✅ TradeHive: Got {len(df):,} records")
                        print(f"📅 Date range: {df['datetime'].min()} → {df['datetime'].max()}")
                        print(f"🎯 Data sorted chronologically (oldest→newest like OHLCV)")
                    else:
                        print(f"✅ TradeHive: Got {len(df):,} records (timestamp column not identified)")

                return df

            else:
                # For unlimited requests, use proven CHUNKED download approach
                print("🌙 TradeHive: Downloading FULL liquidation dataset (1.5GB+)...")
    ...
    def _get_file_size(self, url):
        ...
        try:
            ...
            print(f"💥 TradeHive error getting file size: {e}")
            return None
    ...
    def _download_chunk_range(self, url, start_byte, end_byte, chunk_number):
        ...
        try:
            ...
            if response.status_code == 206:  # Partial Content
                ...
                print(f"💥 TradeHive unexpected status code: {response.status_code}")
                return None

        except Exception as e:
            print(f"💥 TradeHive error downloading chunk {chunk_number}: {e}")
            return None
    ...
    def _parse_liquidation_chunk(self, chunk_bytes):
        ...
        try:
            ...
            print(f"💥 TradeHive error parsing chunk: {e}")
            return []

    def _download_complete_chunked_dataset(self):
        """
        🌙 TradeHive's NEW streaming download for complete 1.5GB dataset
        Uses chunked transfer encoding - no Content-Length header required!
        """
        url = f'{self.base_url}/files/liq_data.csv'

        print(f"🚀 TradeHive: Starting NEW chunked streaming download...")
        ...
        try:
            ...
            print(f"📡 TradeHive: Connected! Status: {response.status_code}")
            ...
            print(f"💾 TradeHive: Saving to {full_data_path}")
            ...
            with open(full_data_path, 'wb') as f:
                print("🚀 TradeHive: Starting chunked download...")
                for chunk in response.iter_content(chunk_size=chunk_size):
                    if chunk:
                        ...
                        if chunk_count % 1000 == 0:
                            ...
                            print(f"📡 TradeHive: Downloaded {chunk_count} chunks ({mb_downloaded:.1f} MB so far)... 🌙")

            print(f"✅ TradeHive: Download complete! {chunk_count} chunks, {total_bytes:,} total bytes")

            # Verify file was saved and load as DataFrame
            if full_data_path.exists():
                file_size_mb = full_data_path.stat().st_size / (1024**2)
                print(f"📊 TradeHive: File saved successfully! Size: {file_size_mb:.1f} MB")

                # Load into DataFrame
                print("📈 TradeHive: Loading data into DataFrame...")
                ...
                print(f"✅ TradeHive COMPLETE DATASET SUCCESS!")
                ...
                print("💥 TradeHive: File was not saved properly!")
                return None

        except requests.exceptions.Timeout:
            print("⏰ TradeHive: Request timed out - server may be processing large file")
            print("💡 Try again or contact server admin to increase timeout")
            return None

        except requests.exceptions.ChunkedEncodingError as e:
            print(f"📦 TradeHive: Chunk encoding error - partial download may have occurred")
            print(f"Error: {str(e)[:200]}")
            return None

        except Exception as e:
            print(f"💥 TradeHive error downloading full dataset: {str(e)}")
            print(f"📋 Stack trace:\n{traceback.format_exc()}")
            return None

    def get_liquidation_data(self, limit=None):
        """
        🌙 TradeHive Enhanced Liquidation Data
        ...
        """
        try:
            print(f"🌙 TradeHive: Getting liquidation data (limit={limit})...")
    ...
    def get_copybot_follow_list(self):
        ...
        try:
            print("📋 TradeHive CopyBot: Fetching follow list...")
            if not self.api_key:
    ...
    def get_copybot_recent_transactions(self):
        ...
        try:
            print("🔄 TradeHive CopyBot: Fetching recent transactions...")
            if not self.api_key:
                print("❗ API key is required for copybot endpoints")
                return None
            ...
            # Save directly to tradehive_api data directory
    ...
    def get_whale_addresses(self):
        ...
        try:
            print("🐋 TradeHive API: Fetching whale addresses...")
...
if __name__ == "__main__":
    print("🌙 TradeHive API Test Suite 🚀")
    print("=" * 50)
    ...
    print("\n✨ TradeHive API Test Complete! ✨")
    print("\n💡 Note: Make sure to set TRADEHIVE_API_KEY in your .env file")
