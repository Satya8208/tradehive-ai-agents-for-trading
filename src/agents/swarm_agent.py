...
Usage:
    ...
    print(result["consensus_summary"])  # 3-sentence synthesis by Claude 4.6
...
# Import TradeHive's model factory
...
SWARM_MODELS = {
    # 🌙 TradeHive's Active Swarm Models - 7 Model Configuration
    "deepseek": (True, "deepseek", "deepseek-v4-flash"),  # DeepSeek V4 Flash - fast model (API)
    ...
    "claude": (True, "claude", "claude-sonnet-4-6"),  # Claude 4.6 Sonnet
    "opus": (True, "claude", "claude-opus-4-7"),  # 🌙 TradeHive - Claude Opus 4.7 - Most powerful!
    ...
    "openrouter_gpt5_mini": (True, "openrouter", "openai/gpt-5.4-mini"),  # GPT-5.4 Mini via OpenRouter
...
MODEL_TIMEOUT = 120  # 🌙 TradeHive - Increased to 120s for more reliability

# Consensus Reviewer - Synthesizes all responses into a clean summary
CONSENSUS_REVIEWER_MODEL = ("deepseek", "deepseek-v4-flash")  # Using DeepSeek Chat API (fast)
...
class SwarmAgent:
    """TradeHive's Swarm Agent for multi-model consensus"""
    ...
    def __init__(self, custom_models: Optional[Dict] = None):
        ...
        cprint("\n" + "="*60, "cyan")
        cprint("🌙 TradeHive's Swarm Agent Initialized 🌙", "cyan", attrs=['bold'])
        cprint("="*60, "cyan")
    ...
    def query(self, prompt: str, system_prompt: Optional[str] = None) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=len(self.active_models)) as executor:
            ...
            try:
                ...
                # 🌙 TradeHive - Don't raise, continue with partial results
...
def main():
    ...
    cprint("\n" + "="*60, "cyan")
    cprint("🌙 TradeHive's Swarm Agent 🌙", "cyan", attrs=['bold'])
    cprint("="*60, "cyan")
