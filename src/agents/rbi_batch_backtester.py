"""
🌙 TradeHive's RBI Batch Backtester (GPT-5 Edition)
...
def ensure_output_dir(research_dir: Path) -> Path:
    ...
    print(f"📂🌙 TRADEHIVE: Output directory ready at: {output_dir} ✨")
    return output_dir
...
def read_strategy(file_path: Path) -> str:
    with open(file_path, "r") as f:
        content = f.read()
    print(f"📝🌙 TRADEHIVE: Loaded strategy text from {file_path.name} ({len(content)} chars) 📚")
    return content
...
def derive_strategy_name(file_path: Path, content: str) -> str:
    ...
    print(f"🏷️🌙 TRADEHIVE: Strategy name resolved: {name}")
    return name


def generate_backtest_code(strategy_text: str) -> str:
    print("🤖🌙 TRADEHIVE: Generating backtest code via BACKTEST prompt... 🚀")
    content = f"Create a backtest for this strategy:\n\n{strategy_text}"

    # Try primary config, then fallbacks if unavailable
    candidate_configs = [
        rbi_agent.BACKTEST_CONFIG,
        {"type": "openai", "name": "gpt-5.4-mini"},
        {"type": "openai", "name": "o3-mini"},
        {"type": "claude", "name": "claude-sonnet-4-6"},
        {"type": "deepseek", "name": "deepseek-v4-flash"},
    ...
    for cfg in candidate_configs:
        print(f"🛰️🌙 TRADEHIVE: Attempting model {cfg['type']} :: {cfg['name']} for codegen ✨")
        try:
            ...
            last_error = str(e)
            print(f"⚠️🌙 TRADEHIVE: Model attempt failed: {last_error}")
    ...
    print("✅🌙 TRADEHIVE: Initial backtest code generated (cleaned from markdown) ✨")
    return code


def package_fix_code(code: str) -> str:
    print("📦🌙 TRADEHIVE: Running package-check to remove forbidden backtesting.lib usage... 🧹")
    ...
    raw = None
    for cfg in candidate_configs:
        print(f"🛰️🌙 TRADEHIVE: Package-fix model attempt {cfg['type']} :: {cfg['name']}")
        try:
            raw = rbi_agent.chat_with_model(
    ...
    print("✅🌙 TRADEHIVE: Package-check completed ✨")
    return fixed or code


def debug_fix_code(code: str, strategy_text: str | None = None) -> str:
    print("🔧🌙 TRADEHIVE: Running debug pass to fix technical issues... 🐛")
    ...
    raw = None
    for cfg in candidate_configs:
        print(f"🛰️🌙 TRADEHIVE: Debug model attempt {cfg['type']} :: {cfg['name']}")
        try:
            raw = rbi_agent.chat_with_model(
    ...
    if not raw:
        return code
    fixed = rbi_agent.clean_model_output(raw, "code")
    print("✅🌙 TRADEHIVE: Debug pass produced updated code ✨")
    return fixed or code
...
def write_code(output_dir: Path, strategy_name: str, code: str) -> Path:
    ...
    with open(out_file, "w") as f:
        f.write(code)
    print(f"💾🌙 TRADEHIVE: Saved backtest code to {out_file}")
    return out_file
...
def save_text_outputs(output_dir: Path, strategy_name: str, stdout: str, stderr: str) -> None:
    ...
    with open(stderr_file, "w") as f:
        f.write(stderr or "")
    print(f"💾🌙 TRADEHIVE: Saved stdout/stderr to {stdout_file.name} / {stderr_file.name}")


def run_one_strategy(research_dir: Path, file_path: Path, conda_env: str) -> None:
    print("\n" + "=" * 80)
    print(f"🌙 TRADEHIVE RUN: Processing {file_path.name} 📜")
    print("=" * 80)
    ...
    while (not result.get("success")) and attempt < MAX_DEBUG_ATTEMPTS:
        attempt += 1
        print(f"❌🌙 TRADEHIVE: Run failed. Attempting auto-fix #{attempt} 🔁")
        # Try debug fix
    ...
    # 4) Finalize outputs
    print("\n📊🌙 TRADEHIVE: EXECUTION RESULTS (raw)")
    ...
    if stdout:
        print("\n📈🌙 TRADEHIVE: STANDARD OUTPUT (stats)")
        print("-" * 60)
        print(stdout)
        print("-" * 60)
    if stderr:
        print("\n⚠️🌙 TRADEHIVE: ERRORS/WARNINGS")
        print("-" * 60)
        print(stderr)
        print("-" * 60)
    ...
    if result.get("success"):
        print("✅🌙 TRADEHIVE: Backtest completed successfully! To the moon! 🚀")
    else:
        print("💥🌙 TRADEHIVE: Backtest still failing after auto-fixes. Needs manual review.")
...
def main():
    ...
    while i < len(args):
        ...
        if arg == "--file" and i + 1 < len(args):
            ...
            i += 1
    print(f"\n🌙 TRADEHIVE: Batch Backtester starting for research dir: {research_dir}")

    # Collect strategy files
    txt_files = sorted([p for p in research_dir.glob("*.txt") if p.is_file()])
    if only_file is not None:
        if not only_file.exists():
            print(f"❌🌙 TRADEHIVE: --file not found: {only_file}")
            return
        if only_file.is_file():
            txt_files = [only_file]
        else:
            print(f"❌🌙 TRADEHIVE: --file must be a file: {only_file}")
            return
    print(f"🔎🌙 TRADEHIVE: Found {len(txt_files)} strategies to process ✍️")

    for idx, file_path in enumerate(txt_files, start=1):
        print(f"\n➡️🌙 TRADEHIVE: [{idx}/{len(txt_files)}] {file_path.name}")
        run_one_strategy(research_dir, file_path, CONDA_ENV_NAME)

    print("\n🎉🌙 TRADEHIVE: Batch run complete. Check the GPT-5 folder for code and outputs. ✨🚀")
