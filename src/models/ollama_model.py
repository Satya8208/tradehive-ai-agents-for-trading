"""
ðŸŒ™ TradeHive's Ollama Model Integration
...
class OllamaModel(BaseModel):
    ...
    AVAILABLE_MODELS = [
        "llama4",          # Meta's latest Llama family model in the Ollama library
    ...
    def __init__(self, api_key=None, model_name="llama4"):
    ...
    def initialize_client(self):
        ...
        try:
            ...
            if response.status_code == 200:
                cprint(f"âœ¨ Successfully connected to Ollama API", "green")
                # Print available models
                models = response.json().get("models", [])
                if models:
                    model_names = [model["name"] for model in models]
                    cprint(f"ðŸ“š Available Ollama models: {model_names}", "cyan")
                    if self.model_name not in model_names:
                        cprint(f"âš ï¸ Model {self.model_name} not found! Please run:", "yellow")
                        cprint(f"   ollama pull {self.model_name}", "yellow")
                else:
                    cprint("âš ï¸ No models found! Please pull the model:", "yellow")
                    cprint(f"   ollama pull {self.model_name}", "yellow")
            else:
                cprint(f"âš ï¸ Ollama API returned status code: {response.status_code}", "yellow")
                raise ConnectionError(f"Ollama API returned status code: {response.status_code}")
        except requests.exceptions.ConnectionError:
            cprint("âŒ Could not connect to Ollama API - is the server running?", "red")
            cprint("ðŸ’¡ Start the server with: ollama serve", "yellow")
            raise
        except Exception as e:
            cprint(f"âŒ Could not connect to Ollama API: {str(e)}", "red")
            cprint("ðŸ’¡ Make sure Ollama is running locally (ollama serve)", "yellow")
            raise
    ...
    def generate_response(self, system_prompt, user_content, temperature=0.7, max_tokens=None, **kwargs):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                cprint(f"âŒ Ollama API error: {response.status_code}", "red")
                cprint(f"Response: {response.text}", "red")
                raise Exception(f"Ollama API error: {response.status_code}")

        except Exception as e:
            cprint(f"âŒ Error generating response: {str(e)}", "red")
    ...
    def get_model_parameters(self, model_name=None):
        ...
        try:
            ...
            cprint(f"âŒ Error getting model parameters: {str(e)}", "red")
            return None
