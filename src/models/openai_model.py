"""
ðŸŒ™ TradeHive's OpenAI Model Implementation
...
class OpenAIModel(BaseModel):
    ...
    AVAILABLE_MODELS = {
        "gpt-5.5": {
            "description": "Latest OpenAI flagship model for complex reasoning, coding, and professional work",
            "input_price": "$5.00/1M tokens",
            "output_price": "$30.00/1M tokens",
            "supports_reasoning_effort": True
        },
        "gpt-5.4": {
            "description": "Affordable OpenAI frontier model for coding and professional work",
            "input_price": "$2.50/1M tokens",
            "output_price": "$15.00/1M tokens",
            "supports_reasoning_effort": True
        },
        "gpt-5.4-mini": {
            "description": "Current mini OpenAI model for coding, computer use, and subagents",
            "input_price": "$0.75/1M tokens",
            "output_price": "$4.50/1M tokens",
            "supports_reasoning_effort": True
        },
        "gpt-5.4-nano": {
            "description": "Lowest-latency current OpenAI GPT-5.4 family model",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.2": {
            "description": "Previous OpenAI GPT-5.2 model for agentic, coding, and multimodal tasks",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.2-chat-latest": {
            "description": "ChatGPT-facing GPT-5.2 model alias for chat-style workloads",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.2-pro": {
            "description": "Higher-compute GPT-5.2 variant for the hardest reasoning tasks",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.2-codex": {
            "description": "GPT-5.2 coding-specialized model for long-horizon agentic coding",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.1": {
            "description": "Previous GPT-5.1 flagship model with configurable reasoning",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.1-codex": {
            "description": "GPT-5.1 coding-specialized model",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
        "gpt-5.1-codex-max": {
            "description": "Higher-compute GPT-5.1 Codex variant for longer coding tasks",
            "input_price": "See OpenAI pricing",
            "output_price": "See OpenAI pricing",
            "supports_reasoning_effort": True
        },
    ...
    def __init__(self, api_key: str, model_name: str = "gpt-5.5", reasoning_effort: str = "medium", **kwargs):
    ...
    def initialize_client(self, **kwargs) -> None:
        ...
        try:
            ...
            cprint(f"âœ¨ TradeHive's magic initialized OpenAI model: {self.model_name} ðŸŒŸ", "green")
            if self._supports_reasoning_effort():
                cprint(f"ðŸ§  Reasoning effort set to: {self.reasoning_effort}", "cyan")
        except Exception as e:
            cprint(f"âŒ Failed to initialize OpenAI model: {str(e)}", "red")
            self.client = None
    ...
    def _prepare_model_kwargs(self, **kwargs):
        ...
        if self._supports_reasoning_effort():
            cprint("ðŸš€ TradeHive's O3 model powering up with reasoning capabilities! ðŸŒ™", "cyan")
    ...
    def generate_response(self, system_prompt, user_content, **kwargs):
        ...
        try:
            ...
            if self.model_name.startswith(('gpt-5', 'o1')):
                try:
                    ...
                    try:
                        ...
                        cprint(f"âŒ Direct Responses HTTP fallback failed: {repr(http_e)}", "red")
            ...
            if self.model_name.startswith('o3'):
                cprint("ðŸ§  Using TradeHive's O3 model with reasoning capabilities...", "cyan")
                messages = [
                    {
                        "role": "user",
            ...
            cprint(f"ðŸ¤” TradeHive's {self.model_name} is thinking...", "yellow")
            ...
            try:
                ...
                cprint(f"ðŸ§ª TradeHive debug: finish_reason={finish_reason}", "cyan")
            except Exception:
                pass
            ...
            if not content_text:
                cprint("ðŸ” Retrying once with simplified prompt format (TradeHive fallback)", "yellow")
                retry_messages = [
                    {
                        "role": "user",
            ...
            if not content_text and hasattr(self.client, 'responses'):
                try:
                    cprint("ðŸ› ï¸ TradeHive fallback: trying Responses API for text output", "yellow")
                    content_str = f"Instructions: {system_prompt}\n\nInput: {user_content}"
                    ...
                    cprint(f"âŒ TradeHive Responses API fallback error: {repr(fallback_e)}", "red")

            # Final safety net: fallback to a smaller current model if nothing came back
            if (not content_text) and self.model_name.startswith('gpt-5') and self.model_name != 'gpt-5.4-mini':
                try:
                    cprint("ðŸ›Ÿ TradeHive fallback: retrying with gpt-5.4-mini to avoid empty content", "yellow")
                    fallback_messages = [
                    ...
                    cprint(f"âŒ Fallback to gpt-5.4-mini failed: {repr(fb_e)}", "red")
            ...
            # Print detailed error info per TradeHive style
            cprint(f"âŒ OpenAI generation error (TradeHive full dump) ðŸš¨: {repr(e)}", "red")
            try:
                cprint(f"ðŸ”Ž type={type(e).__name__}", "yellow")
                if hasattr(e, 'status_code'):
                    cprint(f"ðŸ”Ž status_code={getattr(e, 'status_code', None)}", "yellow")
                if hasattr(e, 'request_id'):
                    cprint(f"ðŸ”Ž request_id={getattr(e, 'request_id', None)}", "yellow")
                if hasattr(e, 'code'):
                    cprint(f"ðŸ”Ž code={getattr(e, 'code', None)}", "yellow")
                if hasattr(e, 'param'):
                    cprint(f"ðŸ”Ž param={getattr(e, 'param', None)}", "yellow")
                resp = getattr(e, 'response', None)
                if resp is not None:
                    cprint(f"ðŸ”Ž response={resp}", "yellow")
                    try:
                        cprint(f"ðŸ”Ž response.body={getattr(e, 'body', None)}", "yellow")
                    except Exception:
                        pass
            except Exception:
                pass
